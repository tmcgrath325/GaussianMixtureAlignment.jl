...
function squared_dist_bounds(
        x::AbstractSinglePointSet, y::AbstractSinglePointSet, σᵣ::Number, σₜ::Number;
...
function squared_dist_bounds(x::AbstractMultiPointSet{N, T, K}, y::AbstractMultiPointSet{N, S, L}, σᵣ, σₜ; kwargs...) where {N, T, K, S, L}
    ...
    for key in keys(x.pointsets) ∩ keys(y.pointsets)
        keylb, keyub = squared_dist_bounds(x.pointsets[key], y.pointsets[key], σᵣ, σₜ; kwargs...)
        lb += keylb
        ub += keyub
    end
    return lb, ub
...
squared_dist_bounds(
    x::AbstractPointSet, y::AbstractPointSet, R::RotationVec, T::SVector{3}, σᵣ::Number, σₜ::Number; kwargs...
) = squared_dist_bounds(R * x, y - T, σᵣ, σₜ; kwargs...)

squared_dist_bounds(
    x::AbstractPointSet, y::AbstractPointSet, ur::UncertaintyRegion; kwargs...
) = squared_dist_bounds(x, y, ur.R, ur.T, ur.σᵣ, ur.σₜ; kwargs...)

squared_dist_bounds(
    x::AbstractPointSet, y::AbstractPointSet, sr::SearchRegion; kwargs...
