...
function branchbound(
        xinput::AbstractModel, yinput::AbstractModel;
        nsplits = 2, searchspace = nothing, blockfun = UncertaintyRegion, initial_rotation = RotationVec(0.0, 0.0, 0.0), initial_translation = SVector{3}(0.0, 0.0, 0.0),
        nextblockfun = lowestlbblock, centerinputs = false, boundsfun = tight_distance_bounds, localfun = local_align, tformfun::TF = AffineMap,
        atol = 0.1, rtol = 0, maxblocks = 5.0e8, maxsplits = Inf, maxevals = Inf, maxstagnant = Inf, separatesplit = false
    ) where {TF}
    x = xinput
    y = yinput
    if isodd(nsplits)
...
function tiv_branchbound(
    ...
    tiv_progress = [
        (rot_res.num_splits + s, obj, (rotparams..., trlp...))
            for (s, obj, trlp) in trl_res.progress
    ]
    push!(tiv_progress, (rot_res.num_splits + trl_res.num_splits, min, bestpos))
    return TIVAlignmentResult(
        x, y, min, trl_res.lowerbound, build_tform(AffineMap, bestpos), bestpos,
