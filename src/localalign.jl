...
function rot_alignment_objective(X::NTuple{3}, gmmx::AbstractModel, gmmy::AbstractModel, block::RotationRegion, args...)
    return alignment_objective((X..., block.T...), gmmx, gmmy, args...)
end

# alignment objective for translation (i.e. the second stage of TIV-GOGMA)
function trl_alignment_objective(X::NTuple{3}, gmmx::AbstractModel, gmmy::AbstractModel, block::TranslationRegion, args...)
    return overlap_alignment_objective((block.R..., X...), gmmx, gmmy, args...)
end


"""
    obj, pos = local_align(x, y, block, pσ=nothing, pϕ=nothing; objfun=alignment_objective, R=nothing, T=nothing, rtol=1e-9, maxevals=100)
...
function local_align(x::AbstractModel, y::AbstractModel, block::SearchRegion=UncertaintyRegion(x,y), args...; 
                     objfun=alignment_objective, maxevals=100)
    ...
    f(X) = objfun(NTuple{ndims}(X), x, y, block, args...)
